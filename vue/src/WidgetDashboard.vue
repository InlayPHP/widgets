...
<template>
  <section aria-label="Dashboard widgets" :class="['text-(--inlay-widget-text)', classNames.root, className]" :data-contract="resource.contract" data-slot="widget-dashboard" :style="style">
    ...
    <div v-if="tabs.length > 1" class="mb-6 flex justify-center overflow-x-auto overflow-y-hidden pb-1" data-slot="dashboard-tabs" role="tablist"><div class="inline-flex max-w-full gap-1 overflow-x-auto rounded-(--inlay-widget-radius) border border-(--inlay-widget-border) bg-(--inlay-widget-muted-surface) p-1 shadow-xs"><button v-for="tab in tabs" :key="tab.name" :aria-selected="tab.name === activeTab" :class="['min-h-(--inlay-button-sm-height) shrink-0 rounded-[calc(var(--inlay-widget-radius)-0.25rem)] px-3 py-1.5 text-sm font-medium transition-[background-color,color,box-shadow] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--inlay-widget-accent)', tab.name === activeTab ? 'bg-(--inlay-widget-surface) text-(--inlay-widget-accent) shadow-xs' : 'text-(--inlay-widget-muted) hover:text-(--inlay-widget-text)']" role="tab" type="button" @click="activeTab = tab.name">{{ tab.label }}</button></div></div>
